...
-- Repair existing rows that have the wrong structure (array, null or missing day keys)
...
WHERE horarios IS NULL
   ...
   OR NOT (horarios ? 'dom' AND horarios ? 'seg' AND horarios ? 'ter'
       AND horarios ? 'qua' AND horarios ? 'qui' AND horarios ? 'sex' AND horarios ? 'sab');