    ...
    'categorias', COALESCE((
      ...
      WHERE c.empresa_id = v_emp.id AND c.ativo = true
        AND EXISTS (
          ...
          WHERE p.categoria_id = c.id AND p.empresa_id = v_emp.id
            AND p.ativo = true AND COALESCE(p.exibir_online, true) = true
        )
    ), '[]'::jsonb),
    'produtos', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', p.id, 'nome', p.nome, 'descricao', p.descricao, 'preco', p.preco,
        'imagem_url', p.imagem_url, 'categoria_id', p.categoria_id,
        'tem_adicionais', p.tem_adicionais
      ) ORDER BY p.nome)
      FROM public.produtos p
      WHERE p.empresa_id = v_emp.id AND p.ativo = true AND COALESCE(p.exibir_online, true) = true
    ), '[]'::jsonb),
    'grupos', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('id', g.id, 'nome', g.nome, 'descricao', g.descricao) ORDER BY g.nome)
