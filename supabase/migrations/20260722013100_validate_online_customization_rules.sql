...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(_itens) LOOP
    ...
    IF v_item ? 'adicionais' AND jsonb_typeof(v_item->'adicionais') = 'array' THEN
      FOR v_ad IN SELECT * FROM jsonb_array_elements(v_item->'adicionais') LOOP
        SELECT a.id, a.nome, a.preco, a.grupo_id, g.nome AS grupo_nome, COALESCE(g.qtd_inclusa,0) AS qtd_inclusa INTO v_adic
          ...
          JOIN public.produto_grupos_adicionais pg ON pg.grupo_id = g.id AND pg.produto_id = v_produto.id
    ...
    IF EXISTS (
      ...
      JOIN public.grupos_adicionais g ON g.id = pg.grupo_id AND g.ativo = true
      WHERE pg.produto_id = v_produto.id
        AND pg.empresa_id = v_emp.id
        AND (
          COALESCE((v_grupo_usado->>g.id::text)::int, 0) < GREATEST(CASE WHEN g.obrigatorio THEN 1 ELSE 0 END, g.min_selecao)
          OR (NOT g.max_ilimitado AND COALESCE((v_grupo_usado->>g.id::text)::int, 0) > g.max_selecao)
        )
    ) THEN
      RAISE EXCEPTION 'Sele??es de adicionais inv?lidas para o produto %.', v_produto.nome;
    END IF;
